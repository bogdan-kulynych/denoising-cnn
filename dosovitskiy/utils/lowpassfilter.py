...
def lowpassfilter(img, sigma, batchsize=100, type='gaussian', verbose='false'):
    if img.shape[0] > 1 and img.shape > 1:
        imsize = img.shape
        img = np.reshape(img, (imsize[0], imsize[1]))
        out = np.zeros(img.shape,dtype=img.dtype)

        dx=round(2 * sigma);
        tmpimg=[img(:,dx:-1:1,:) img  img(:, size(img, 2):-1:size(img, 2) - dx + 1,:)];
        tmpimg = [tmpimg(dx:-1:1,:,:); tmpimg;
        tmpimg(size(img, 1):-1:size(img, 1) - dx + 1,:,:)];
        if verbose
        ...
        % lowpass = zeros(size(tmpimg, 1), size(tmpimg, 2));
        % lowpass(1:2, 1:2)=1;
        lowpass(1:2, end - 1:end)=1;
        lowpass(end - 1:end, 1:2)=1;
        lowpass(end - 1:end, end - 1:end)=1;
    ...
    dx = round(2 * sigma);
    tmpimg = [img(dx:-1:1,:); img;
    img(size(img, 1):-1:size(img, 1) - dx + 1,:)];
